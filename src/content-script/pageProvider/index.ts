...
const log = (event, ...args) => {
  if (process.env.NODE_ENV !== 'production') {
    // console.log(
    //   `%c [mach] (${new Date().toTimeString().slice(0, 8)}) ${event}`,
...
const channelName = script?.getAttribute('channel') || 'NACH';
...
export class MatchProvider extends EventEmitter {
...
const provider = new MatchProvider();

if (!window.mach) {
  window.mach = new Proxy(provider, {
    deleteProperty: () => true
  });
}

Object.defineProperty(window, 'mach', {
...
window.dispatchEvent(new Event('mach#initialized'));
